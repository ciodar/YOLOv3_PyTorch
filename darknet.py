...
class Darknet(nn.Module):
    ...
    def __init__(self, cfgfile, use_cuda=True):
        ...
        try:
            self.condition = bool(self.blocks[0]['condition'])
        except:
    ...
    def forward(self, x):
        ...
        for block in self.blocks:
            ...
            if block['type'] == 'net':
                ...
                layers = [int(i) if int(i) > 0 else int(i)+ind for i in layers]
                x = outputs[layers[0]]
                gamma = outputs[layers[1]]
                gamma = 1 - gamma
                beta = outputs[layers[2]]
    ...
    def load_weights(self, weightfile):
        ...
        for block in self.blocks:
            ...
            if block['type'] == 'net':
                ...
                #remember change this when load weight file from condition
                if self.condition:
                    # print('Do not to load connected weight for condition network')
                    pass
                else:
                    model = self.models[ind]
                    if block['activation'] != 'linear':
        ...
        for blockId in range(1, cutoff + 1):
            ...
            if block['type'] == 'convolutional':
                ...
                if batch_normalize:
                    ...
                    count_no_bn += 1
                    # print('no bn: ',count_no_bn,' ',ind)
                    ### this is if we want to save weight from the model trained on fewer class to model will train more class
                    ### so we save the last fc (before yolo detection layer with more value by repeat some times.
                    if (targetclass != num_class):
                        save_conv_target_class(fp, model[0], targetclass, num_class)
                    else:
                        save_conv(fp, model[0])
            elif block['type'] == 'connected':
                model = self.models[ind]
